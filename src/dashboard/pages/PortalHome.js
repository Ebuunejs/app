...
function PortalDashboard() {  
  ...
  export default PortalDashboard
     ...
     /*
            const responseBooking = await axios.get(`${BASE_URL}/bookings`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });

            console.log("TimeSlots: ",responseBooking);
            if (responseBooking.status === 200) {
                for(let i=0; i < responseBooking.data.length;i++){
                    const idTimeslot= responseBooking.data[i]['timeslots_id'];
                    console.log(idTimeslot)
                    
                    const responseTimeslot=await axios.get(`${BASE_URL}/timeslots/${idTimeslot}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
                    
                    console.log("Response: ",responseTimeslot)
                    const client=responseBooking.data[i]['clients_id'];
                    console.log("client",client)
                    
                    const responseUser = await axios.get(`${BASE_URL}/clients/${client}`, {
                        headers: {
                            'Authorization': `Bearer ${token}`
                        }
                    });
                    ...
                    const date= responseTimeslot.data['date'];
                    const time=responseTimeslot.data['time'];
                    let startDate=`${date} ${time}`;
                    startDate= new Date(startDate);
                    ...
                    console.log("Time: ",startDate)
                    console.log("EndTime:",endDate)
                    console.log("Response Clients",responseUser)
                    timeSlots[i]={  id:     responseTimeslot.data['id'],
                                    title:  responseUser.data['name'] + " "+responseUser.data['surname'],
                                    start:  startDate,
                                    end:    endDate,
                                    desc:   responseUser.data['phone']
                                }
                }
